...
To also derive `fields` on the implicit unboxed version of a record, add the
`~unboxed` flag. Getters for an implicit unboxed version are mangled with a "_u"
suffix to distinguish them from the boxed version (there are no setters because
all unboxed record fields are immutable). All other definitions are put in a
module named `Fields_of_<typename>_u` (or `Fields_u` if `<typename>` is `t`).

See [Full list of selectors](#full-list-of-selectors) below for an example.
...
to produce an error during preprocessing. Note that the default behavior derives
`map_poly`, so it doesn't work with records with unboxed fields.
...
(* getters for t# *)
val cancelled_u : t# -> bool @@ portable [@@zero_alloc]
val price_u     : t# -> float @@ portable [@@zero_alloc]
val quantity_u  : t# -> int @@ portable [@@zero_alloc]
val dir_u       : t# -> [ `Buy | `Sell ] @@ portable [@@zero_alloc]

(* local getters for t# *)
val cancelled_u__local : local_ t# -> local_ bool @@ portable [@@zero_alloc]
val price_u__local     : local_ t# -> local_ float @@ portable [@@zero_alloc]
val quantity_u__local  : local_ t# -> local_ int @@ portable [@@zero_alloc]
val dir_u__local       : local_ t# -> local_ [ `Buy | `Sell ] @@ portable [@@zero_alloc]
...
module Fields_u : sig
  (* similar to the signature of [Fields], but every [t] has replaced with a
     [t#] and [set_all_mutable_fields] doesn't mutate any fields *)
  ...
  module Direct : sig
      ...
      val set_all_mutable_fields : local_ t# -> unit @@ portable [@@zero_alloc]
    end
end
```
<!--END-->
...
the maintainers if you have an example of a case where this would be useful.
