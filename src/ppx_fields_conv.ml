  ...
  fun (lbls : label_declaration list) ->
    let generated_funs =
      let extra_forbidden_names =
        List.filter_map lbls ~f:(function
          | { pld_mutable = Mutable; pld_name; _ } ->
              Some ("set_" ^ pld_name.txt)
          | _ -> None)
      in
    ...
    List.filter_map lbls ~f:(fun { pld_name; pld_loc; _ } ->
        if List.mem generated_funs pld_name.txt ~equal:String.equal then
          Some
            (Location.Error.createf ~loc:pld_loc
               "ppx_fields_conv: field name %S conflicts with one of the \
                generated functions"
               pld_name.txt)
        else None)
...
module Inspect = struct
  let field_names labdecs =
    List.map labdecs ~f:(fun labdec -> labdec.pld_name.txt)
end
...
let check_at_least_one_record ~loc rec_flag tds =
  ...
  if not (List.exists tds ~f:is_record) then
    Location.raise_errorf ~loc
      ...
      | _ ->
          "'with fields' can only be applied on type definitions in which at \
...
let assemble ~loc ~selection ~fields_module ~make_module ~make_error alist =
  let alist =
    List.filter alist ~f:(fun (selector, _) -> Set.mem selection selector)
  in
      ...
      [
        make_error
          (Location.Error.createf ~loc
             "[@@deriving fields]: no definitions generated");
      ]
  | false ->
      ...
      List.concat
        [
          inline;
          module_defn ~loc ~make_module ~name:fields_module
            (List.concat
               [ fields; module_defn ~loc ~make_module ~name:"Direct" direct ]);
        ]
...
module Gen_sig = struct
  ...
  let fold_fun ~private_ ~ty_name ~tps ~loc labdecs =
    ...
    let f i arg : arg_label * core_type =
      field_arg ~loc ~private_ ~record
        (fun ~field ~ty:_ ->
          [%type: [%t acc i] -> [%t field] -> [%t acc (i + 1)]])
        arg
    in
    let types = List.mapi labdecs ~f in
    let init_ty = label_arg "init" (acc 0) in
    let t =
      Create.lambda_sig ~loc (init_ty :: types) (acc (List.length labdecs))
    in
  ...
  let fold_right_fun ~private_ ~ty_name ~tps ~loc labdecs =
    ...
    let f i arg : arg_label * core_type =
      field_arg ~loc ~private_ ~record
        (fun ~field ~ty:_ ->
          [%type:
            [%t field] -> [%t acc (numlabs - i - 1)] -> [%t acc (numlabs - i)]])
        arg
    in
    let types = List.mapi labdecs ~f in
    let init_ty = label_arg "init" (acc 0) in
  ...
  let direct_to_list_fun ~private_ ~ty_name ~tps ~loc labdecs =
    ...
    let types = List.map labdecs ~f in
    let t =
      Create.lambda_sig ~loc ((Nolabel, record) :: types) [%type: 'elem__ list]
    in
    A.sig_item ~loc "to_list" t
  ...
  let map_poly ~private_ ~ty_name ~tps ~loc _ =
    ...
    let tps_names =
      List.map tps ~f:(fun tp ->
          match tp.ptyp_desc with Ptyp_var var -> var | _ -> assert false)
    in
    let fresh_variable =
      let rec loop i =
  ...
  let set_all_mutable_fields ~ty_name ~tps ~loc labdecs =
    ...
    let labels =
      List.fold_right labdecs ~init:[%type: unit] ~f:(fun labdec acc ->
          ...
          | Immutable -> acc
          | Mutable ->
              ptyp_arrow ~loc (Labelled labdec.pld_name.txt) labdec.pld_type acc)
    in
    A.sig_item ~loc "set_all_mutable_fields" [%type: [%t record] -> [%t labels]]

  let record ~private_ ~ty_name ~tps ~loc ~selection
      (labdecs : label_declaration list) : signature =
    let fields =
      List.rev_map labdecs ~f:(fun labdec ->
          ...
          let field =
            A.sig_item ~loc name (field_t ~loc private_ [ record_ty; ty ])
          in
    ...
    let direct_fold_right =
      direct_fold_right_fun ~private_ ~ty_name ~tps ~loc labdecs
    in
    let direct_map = direct_map_fun ~ty_name ~tps ~loc labdecs in
    let direct_and_f =
      direct_bool_fun "for_all" ~private_ ~ty_name ~tps ~loc labdecs
    in
    let direct_or_f =
      direct_bool_fun "exists" ~private_ ~ty_name ~tps ~loc labdecs
    in
    let direct_to_list =
      direct_to_list_fun ~private_ ~ty_name ~tps ~loc labdecs
    in
    let set_all_mutable_fields =
      set_all_mutable_fields ~ty_name ~tps ~loc labdecs
    in
    List.concat
      [
        getters_and_setters;
    ...
    |> assemble ~loc ~selection ~fields_module ~make_module:A.sig_mod
  ...
  let fields_of_td (td : type_declaration) ~selection : signature =
    ...
    | Ptype_record labdecs -> (
        ...
        | _ ->
            List.map list_of_collisions ~f:(fun error ->
                psig_extension ~loc (Location.Error.to_extension error) []))
    | _ -> []
...
module Gen_struct = struct
      ...
      let getter =
        ( Selector.Per_field Getters,
          A.str_item ~loc name
            [%expr
              fun _r__ ->
                [%e pexp_field ~loc [%expr _r__] (Located.lident ~loc name)]] )
      in
        ...
        | Mutable, Private ->
            ( [],
              [%expr
                Some
                  (fun _ _ ->
                    failwith "invalid call to a setter of a private type")] )
        | Mutable, Public ->
            let setter =
              ( Selector.Per_field Setters,
                A.str_item ~loc ("set_" ^ name)
                  [%expr
                    fun _r__ v__ ->
                      [%e
                        pexp_setfield ~loc [%expr _r__]
                          (Located.lident ~loc name) [%expr v__]]] )
            in
      ...
      let field =
        ...
        let fset =
          ...
          | Private ->
              [%expr
                fun _ _ ->
                  failwith "Invalid call to an fsetter of a private type"]
  ...
  let label_arg ?label ~loc name =
    let l = match label with None -> name | Some n -> n in
  ...
  let creation_fun ~loc _record_name labdecs =
    ...
    let f =
      ...
      in
      let body =
        List.fold_right names ~init:[%expr [%e body_record]]
                ...
                ~expr:
                  [%expr
                    [%e evar ~loc (field_name ^ "_fun__")]
                      [%e evar ~loc field_name] compile_acc__];
            ]
            acc)
    in
    let f =
  ...
  let fold_fun ~loc labdecs =
    ...
    let field_fold acc_expr field_name =
      [%expr
        [%e evar ~loc (field_name ^ "_fun__")]
          [%e acc_expr] [%e evar ~loc field_name]]
    in
  ...
  let direct_fold_fun ~loc labdecs =
    ...
    let field_fold acc_expr field_name =
      [%expr
        [%e evar ~loc (field_name ^ "_fun__")]
          [%e acc_expr] [%e evar ~loc field_name] record__
  ...
  let fold_right_fun ~loc labdecs =
    ...
    let field_fold_right field_name acc_expr =
      [%expr
        [%e evar ~loc (field_name ^ "_fun__")]
          [%e evar ~loc field_name] [%e acc_expr]]
    in
  ...
  let direct_fold_right_fun ~loc labdecs =
    ...
    let field_fold_right field_name acc_expr =
      [%expr
        [%e evar ~loc (field_name ^ "_fun__")]
          [%e evar ~loc field_name] record__
          [%e pexp_field ~loc [%expr record__] (Located.lident ~loc field_name)]
          [%e acc_expr]]
    in
    let body = List.fold_right names ~f:field_fold_right ~init:[%expr init__] in
  ...
  let and_fun ~loc labdecs =
    ...
    let field_fold acc_expr field_name =
      [%expr
        [%e acc_expr]
        && [%e evar ~loc (field_name ^ "_fun__")] [%e evar ~loc field_name]]
    in
    ...
    let field_fold acc_expr field_name =
      [%expr
        ...
        && [%e evar ~loc (field_name ^ "_fun__")]
             [%e evar ~loc field_name] record__
             [%e
               pexp_field ~loc [%expr record__] (Located.lident ~loc field_name)]]
    in
    let body = List.fold_left names ~init:(ebool ~loc true) ~f:field_fold in
    let patterns = List.map names ~f:(label_arg_fun ~loc) in
    let lambda =
      Create.lambda ~loc ((Nolabel, [%pat? record__]) :: patterns) body
    in
    A.str_item ~loc "for_all" lambda
  ...
  let map_poly ~loc labdecs =
    ...
    in
    let body = List.fold_right names ~init:[%expr []] ~f:fold in
    A.str_item ~loc "map_poly"
      (pexp_fun ~loc Nolabel None [%pat? record__] body)

  let sequence_ ~loc xs =
    match List.rev xs with
    | [] -> [%expr ()]
    | x :: xs ->
        List.fold_left ~init:x xs ~f:(fun x y -> pexp_sequence ~loc y x)
  ...
  let set_all_mutable_fields ~loc labdecs =
      ...
      let exprs =
        List.fold_right labdecs ~init:[] ~f:(fun labdec acc ->
            ...
            | Mutable ->
                ...
                pexp_setfield ~loc (evar ~loc record_name)
                  (Located.lident ~loc field_name)
                  (evar ~loc field_name)
                :: acc)
      in
      ...
      *)
      [%expr
        let [%p pvar ~loc record_name] =
          Fieldslib.Field.For_generated_code.opaque_identity
            [%e evar ~loc record_name]
        in
    ...
    let function_ =
      List.fold_right labdecs ~init:body ~f:(fun labdec acc ->
          ...
          | Mutable ->
              let field_name = labdec.pld_name.txt in
              pexp_fun ~loc (Labelled field_name) None (pvar ~loc field_name)
                acc)
    in
    let body = pexp_fun ~loc Nolabel None (pvar ~loc record_name) function_ in
    [%stri let[@inline always] set_all_mutable_fields = [%e body]]

  let record ~private_ ~record_name ~loc ~selection
      (labdecs : label_declaration list) : structure =
    ...
    let fields_module =
      if String.equal record_name "t" then "Fields"
      else "Fields_of_" ^ record_name
    in
...
let fields =
  Deriving.add "fields"
    ~str_type_decl:
      (Selector.generator Gen_struct.generate ~add_dependencies:true)
